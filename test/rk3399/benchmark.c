...
static REAL linpack  (uint32_t cpuid, long nreps,int arsize);
...
static void *mempool[16];


void benchmark(uint32_t cpuid, int arsize)
    ...
    while (1)
        ...
        if (malloc_arg!=memreq || (mempool[cpuid]=sys_malloc_align(malloc_arg, 128))==NULL)
            {
        ...
        printf("----------------------------------------------------\n");
        nreps=1;
        while (linpack(cpuid, nreps,arsize)<10.)
            nreps*=2;
        sys_free(mempool[cpuid]);
        printf("\n");
        }
    }
...
static REAL linpack(uint32_t cpuid, long nreps,int arsize)
    ...
    if (toverhead<0.)
        toverhead=0.;
    uint32_t arg1 = totalt;
 	uint32_t arg2 = 100.*tdgefa/totalt;
	uint32_t arg3 = 100.*tdgesl/totalt;
	uint32_t arg4 = 100.*toverhead/totalt;
	uint32_t arg5 = kflops;
    uint32_t arg1f = (uint64_t)(totalt*100) % 100;
 	uint32_t arg2f = (uint64_t)(100.*tdgefa/totalt) % 100;
	uint32_t arg3f = (uint64_t)(100.*tdgesl/totalt) % 100;
	uint32_t arg4f = (uint64_t)(100.*toverhead/totalt) % 100;
	uint32_t arg5f = (uint64_t)(kflops) % 100;

	printf("%d:%8d ", (uint32_t)CPUID_get(), (int)nreps);
	printf("%6d.%02d ", arg1, arg1f);
	printf("%6d.%02d%% ", arg2, arg2f);
	printf("%6d.%02d%% ", arg3, arg3f);
	printf("%6d.%02d%%  ", arg4, arg4f);
	printf("%9d.%02d\n", arg5, arg5f);
    return(totalt);
    }
...
static REAL second(void)
    ...
    {
	return (REAL)CNTPCT_EL0_get() / (REAL)CNTFRQ_VALUE;
